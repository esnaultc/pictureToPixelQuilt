...
add_params <- function(cfg_i, img_i) {
	...
	cfg_i[['tile_width']] <- list(
		'value' = floor(cfg_i[['img_width']][['value']] /
	...
	cfg_i[['tile_number_height']] <- list(
		'value' = floor(cfg_i[['img_height']][['value']] / cfg_i[['tile_width']][['value']]),
...
For easier quilt assembly, the full picture is divided in
`r cfg[['grid_block_size']][['value']]`x`r cfg[['grid_block_size']][['value']]` tiles blocks.
...
grid_x_breaks <- seq(0.5, cfg[['tile_number_width']][['value']],
	 by = cfg[['grid_block_size']][['value']])
grid_y_breaks <- seq(0.5, cfg[['tile_number_height']][['value']],
	 by = cfg[['grid_block_size']][['value']])
...
For easier assembly, first sew together `r cfg[['grid_block_size']][['value']]` tiles x 
`r cfg[['grid_block_size']][['value']]` tiles blocks, then sew rows of blocks, and
...
Below are instructions for making each of the
`r cfg[['grid_block_size']][['value']]`x`r cfg[['grid_block_size']][['value']]` blocks.
